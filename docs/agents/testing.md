...
| `api-route.server.test.ts`          | `apiPost`: cada error al código HTTP que ve el móvil (JSON no válido 400, `Unauthorized` 401, `RateLimitError` 429 con `retry-after`, `ValidationError` 400, el resto 500 genérico)                                                                                                                                                                                                                                                                                                  |
| `rate-limit.server.test.ts`         | con el doble: tope de gasto diario/mensual y `capScope`, cuota por `user:<id>`, los _fail-open_ con su evento (`spend_cap_failopen`, `rate_limit_failopen`, `spend_record_failed`), el correo solo como hash                                                                                                                                                                                                                                                                         |
| `household.server.test.ts`          | con el doble y tres hogares: `householdContext` (solo su hogar, planificador, comidas heredadas, raciones sin el bebé de triturados, reintento sin columnas opcionales) y `householdPlannerId`                                                                                                                                                                                                                                                                                       |
| `push-dispatch.server.test.ts`      | con el doble, reloj fijo y `send` inyectado: ventana de mañana/noche por zona horaria, ya enviado, sin suscripción, `gone`/`failed`/error, tono, aviso de preparar el mes que viene                                                                                                                                                                                                                                                                                                  |
| `account.functions.test.ts`         | `deleteAccountHandler` borra siempre el usuario de la sesión, nunca un id del cliente                                                                                                                                                                                                                                                                                                                                                                                                |
| `ai-provider.server.test.ts`        | `asPromptData`: saltos de línea, `«»`, fences, invisibles, `null`, números, recorte                                                                                                                                                                                                                                                                                                                                                                                                  |
| `chat-preflight.test.ts`            | orden de `/api/chat`: fuera de alcance no lee la clave ni gasta cuota; sin clave no toca la cuota; 429                                                                                                                                                                                                                                                                                                                                                                               |
| `test/fake-supabase.test.ts`        | el propio doble de Supabase frente a PostgREST, y que `supabaseAdmin` nunca es el real en un test                                                                                                                                                                                                                                                                                                                                                                                    |
