...
export type DailyLog = {
  ...
  exercise?: import("./exercise").DayExercise | null;
  /**
   ...
   */
  adjustment?: import("./day-balance").DayAdjustmentRecord | null;
};
...
/**
 * Deriva la dirección del objetivo a partir del peso actual y el peso objetivo.
 * Se usa tanto para los prompts de la IA como para el onboarding, donde
 * reemplaza la selección manual de `goal_type`.
 */
...
export function goalProgress(profile: Profile | null): GoalProgress {
  ...
  // -- Camino nuevo: target_weight_kg es la fuente canonica --
  if (profile.target_weight_kg != null) {
    ...
    // La direccion del PROGRESO se mide contra el camino original (start->target),
    // no contra la posicion actual. Asi, si alguien empezo en 100 queriendo 80 y
    // ahora esta en 78 (sobrepaso), sigue viendo 100% de progreso en vez de un
    // cambio repentino a "ganar". `deriveGoalType` (current vs target) es para
    // los prompts de la IA — decide que hacer AHORA, no el historico.
    ...
    if (originalDir === "mantener" || totalPath < 1) {
      // Start ~ target: la persona quiere mantenerse donde ya esta.
      return {
    ...
    // "perder" o "ganar": progreso = fraccion del camino start->target recorrida.
  ...
  // -- Fallback legacy: goal_type + goal_amount (usuarios pre-migracion) --
  ...
  // "habitos" / "energia": no hay metrica de peso que ensenar.
  return ZERO;
}
