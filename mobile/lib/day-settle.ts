...
import type { DayOutcome } from "./day-balance";
import type { MealChange, MealSlot } from "./plan-shared";
 ...
 * `use-meal-swap.ts`. Tres timers de 10 s, tres claves de AsyncStorage, tres
 * suscripciones a `AppState` y tres llamadas a la IA que se peleaban por los
 * mismos días futuros (ver `day-balance.ts`). Aquí hay uno.
 *
 * Cualquier cosa que desvíe el día —cambiar un plato, picotear, hacer deporte—
 * reinicia la misma ventana de calma y acaba en UNA llamada a
 * `POST /api/v1/day/settle`, que suma el día entero y decide una vez.
 *
 ...
 * Que haya varias pasadas en un día no rompe nada porque el servidor las
 * acumula (`mergeDayAdjustment`).
 *
 * Copia de `src/lib/day-settle.ts` de la web, con AsyncStorage en vez de
 * `localStorage` y `AppState` en vez de `visibilitychange`.
 */
...
function wireFlushOnBackground() {
  ...
  AppState.addEventListener("change", (state) => {
    if (state === "background" || state === "inactive") flushDaySettle();
  });
}
 ...
 * quien desvía el día sin pasar por Hoy: el registro guiado del chat abierto
 * con un enlace directo. Sin ellas el lote esperaría a que se abriera Hoy, y si
 * no se abre ese día, el pendiente se tiraría sin compensar. No saben calcular
 * platos (eso es de `use-meal-swap`): uno encolado vuelve a la cola hasta que
 * Hoy registre las suyas, que siempre mandan sobre estas. Copia de la web.
 */
...
async function resume(today: string): Promise<void> {
  ...
  try {
    ...
    const stale = keys.filter(
      (k) => k.startsWith(STORAGE_PREFIX) && k !== `${STORAGE_PREFIX}${today}`,
    );
