...
export type Profile = {
  ...
  /** Slots elegidos en el onboarding (issue 04) — ver `effectiveMealSlots` en
...
export async function saveProfile(patch: Partial<Profile>) {
  ...
  const { error } = await supabase
    .from("profiles")
    .upsert({ id: userId, ...next } as never, { onConflict: "id" });
  if (error) throw error;
}
...
/**
 * Cambia las comidas del registro de hoy **releyéndolas de la base de datos
 * justo antes de escribir**, en vez de mandar un array que el cliente tenía en
 * memoria.
 *
 * `habits` es una única columna JSON, así que cada guardado reescribe la lista
 * entera: dos operaciones lentas solapadas (cambiar dos platos seguidos, cada
 * uno con su llamada a la IA de por medio) hacían que la última pisara a la
...
export async function logTodayWeight(kg: number) {
  ...
  if (!data || data.length === 0) {
    ...
    // se descartó en el onboarding (`reconcileHabits` lo corrige después, pero
    // más vale no ensuciar el dato de entrada).
