...
export function validateSyncHouseholdPlan(input: { month: string; today?: string }) {
 ...
 */
export async function syncHouseholdPlanCore(
  auth: RequestAuth,
  data: ReturnType<typeof validateSyncHouseholdPlan>,
...
export const syncHouseholdPlan = createServerFn({ method: "POST" })
  ...
  .inputValidator(validateSyncHouseholdPlan)
  .handler(({ data, context }) => syncHouseholdPlanCore(context, data));
